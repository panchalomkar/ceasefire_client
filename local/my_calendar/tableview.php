...
class tableview extends table_sql {
    ...
    function __construct($uniqueid) {
        ...
        $headers = array(
            get_string('coursename', LANGUAGE),
            get_string('startdate', LANGUAGE),
            get_string('department', LANGUAGE),
            get_string('status', LANGUAGE)
        );
    ...
    function col_coursestatus($values) {
        ...
        if ($values->timecompleted > 0) {
            return get_string('completed', LANGUAGE);
        } if ($values->timestarted > 0) {
            return get_string('inprogress', LANGUAGE);
        } else {
            return get_string('notstarted', LANGUAGE);
        }
    }
