...
trait get_livesession_detail {
    ...
    public static function get_livesession_detail($userid) {
        ...
        $fivedaysbeforedate = strtotime(date('d-m-Y', strtotime("+7 days",$currentdate)));
      ...
      foreach ($instanceid as $value) {   
        ...
        $dataget[] = array(
             'activityname' => $value->name.' '.$value->modulename,
             ...
             'imgurl' => $CFG->wwwroot.'/theme/remui/pix/demo-img.png'
         );
       }
       }
 
       $response = array(
        "livesessiondetail" => json_encode($dataget)
    );
