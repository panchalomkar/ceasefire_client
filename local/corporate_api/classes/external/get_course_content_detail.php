...
trait get_course_content_detail {
    ...
    public static function get_course_content_detail($courseid, $userid, $options = array()) {
        ...
        $inprogressactivity = count($course_modules) - count($getcmcom);

        $datagetss = array(
            'totalactivity' => count($course_modules),
            'completedactivity' => count($getcmcom),
            'inprogressactivity' => $inprogressactivity,
        );
       
        $response = array(
            "activitystatus" => json_encode($datagetss),
            "course_content" => json_encode($coursecontents)
        );
        return $response;
    }
