...
echo '<div class="header-btns d-flex py-3 justify-content-between">';
    ...
    echo '<div class="d-flex">
            <form action="'.$PAGE->url.'" class="d-flex">
                ...
                <input type="submit" class="btn btn-primary" value="'.get_string('submit').'" />
            </form>
        </div>';
    if(is_siteadmin()){
        echo '<div class="d-flex">
...
$table->head = array('User Pic', 'Fullname', 'Email','Course Enrolled','Course Inprogress', 'Course Completed');
...
foreach ($teamusers as $record) {
    ...
    $table->data[] = array('<img src="'.$pic.'" />', $fullname, $email, $courseenrolled,$courseinprogress, $coursecompleted);
}
