...
if ((has_capability('local/venuemanangement:managevenue', $context)) || (has_capability('local/venuemanangement:viewvenuelist', $context))) {
    ...
    if ($classroomfilter != '') {
        ...
        if (!empty($venuemanangement_detail)) {
            $filter_params['classroom'] = $classroomfilter;
        } else {
...
//        $flag = $i;}
    ...
    if (!empty($table)) {
        ...
        $formcontent .= 'Search by Classroom';
        $formcontent .= html_writer::end_tag('label');
        $formcontent .= html_writer::empty_tag('input', array('type' => 'text', 'name' => 'classroom', 'value' => $classroomfilter));
        $formcontent .= html_writer::end_tag('div');
