...
if ($confirm && $suspend) 
{
    ...
    $notifications = get_string('users_suspended','local_people');
    foreach ($rs as $user) 
    {
      ...
      unset($SESSION->bulk_users);
       ...
       $notifications .=trim($userssuspende);
        ...
        // users suspended
            ...
            // users that cant be suspended
                echo $OUTPUT->box_start('generalbox mar-top ', 'notice');
                if (!empty($notifications)) {
                    echo get_string('users_cant_be_suspended','local_people').$usersnotsuspende;
                }
                echo $OUTPUT->box_end();
            }
