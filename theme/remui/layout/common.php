...
$mycustommenus = [];
$customkey = 0;
foreach ($flatnavigation as $index => $navs) {
    $navs->show = true;
    // Code for custom navigation.
 //  print_object($navs);die;
    if (count($navs->children)) {
        $navs->haschild = true;
    }
    if ($navs->key == "custommenu" || $navs->parent->key == 'custommenu') {

        if ($navs->key == 'customsubmenu') {
            $navs->parent->key = 'custommenu_' . $customkey;
            $navs->iscustommenu = true;
            $navs->ischildnode = true;
            $navs->show = false;
            // $templatecontext['showcustommenu'] = true;
        }
        if ($navs->key == "custommenu") {
            $customkey++;
            $navs->ischildnode = false;
            $navs->iscustommenu = true;

            $navs->key = 'custommenu_' . $customkey;
        }
    }
   
    // if (isset($navs->parent->key) && $navs->parent->key == 'custommenu') {
    //     if ($navs->key = "customsubmenu") {
    //         $templatecontext['showcustosubmenu'][] = $navs;
    //         $templatecontext['showcustommenu'] = true;
    //     }
    // }
}
