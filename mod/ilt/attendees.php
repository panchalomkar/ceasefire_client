...
if ($canviewattendees || $cantakeattendance) {
    ...
    if (empty($attendees)) {
        ...
        if ($takeattendance) {
            ...
            echo html_writer::empty_tag('input', array('type' => 'submit', 'value' => get_string('saveattendance', 'ilt')));
            ...
            if ($cantakeattendance && $session->datetimeknown && ilt_has_session_started($session, time())) {
                ...
                echo html_writer::link($attendanceurl, get_string('takeattendance', 'ilt')) . ' - ';
            }
        }
    }
    ...
    if (!$takeattendance) {
        if (has_capability('mod/ilt:addattendees', $context) ||
            ...
            echo html_writer::link($editattendeeslink, get_string('addremoveattendees', 'ilt')) . ' - ';
        }
    }
}
...
echo html_writer::link($url, get_string('goback', 'ilt')) . html_writer::end_tag('p');
