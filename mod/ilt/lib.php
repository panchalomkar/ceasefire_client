...
function ilt_write_worksheet_header(&$worksheet) {
    ...
    $worksheet->write_string(0, $pos++, get_string('date', 'ilt'));
    ...
    foreach ($sessions as $session) {
        ...
        if (!empty($sessionsignups[$session->id])) {
            foreach ($sessionsignups[$session->id] as $attendee) {
                $i++;
                $j = ilt_write_activity_attendance_helper($worksheet, $i, $session, $customsessionfields, $status, $dateformat, $starttime, $finishtime);
                if ($trainerroles) {
...
function ilt_write_activity_attendance_helper(&$worksheet, $i, $session, $customsessionfields, $status, $dateformat, $starttime, $finishtime) {
    $j = 0;
 ...
 *
 */
function ilt_get_instructor( $instructor) {
    global $CFG, $DB;
    $instarray = array();
...
class ilt_candidate_selector extends user_selector_base {
    ...
    public function find_users($search) {
        ...
        $sql = "
                 ...
                 WHERE $wherecondition AND e.courseid = $this->courseid
                   AND u.id NOT IN
                       (
                       SELECT u2.id
                         FROM {ilt_signups} s
