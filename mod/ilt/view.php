...
$PAGE->set_url('/mod/ilt/view.php', array('id' => $cm->id));
...
echo $OUTPUT->heading(get_string('allsessionsin', 'ilt', $ilt->name), 2);
...
print_session_list($course->id, $ilt->id, $location);
...
if (has_capability('mod/ilt:viewattendees', $context)) {
    ...
    echo html_writer::empty_tag('input', array('type' => 'submit', 'value' => get_string('exporttofile', 'ilt')));
    echo html_writer::end_tag('div'). html_writer::end_tag('form');
}
...
function print_session_list($courseid, $iltid, $location) {
    ...
    if ($sessions = ilt_get_sessions($iltid, $location) ) {
        foreach ($sessions as $session) {
            ...
            $sessionstarted = ilt_has_session_started($session, $timenow);
            if ($session->datetimeknown && $sessionstarted && ilt_is_session_in_progress($session, $timenow)) {
                $sessionstarted = true;
    ...
    // Upcoming sessions.
    echo $OUTPUT->heading(get_string('upcomingsessions', 'ilt'));
    ...
    if (!empty($previousarray)) {
        echo $OUTPUT->heading(get_string('previoussessions', 'ilt'));
