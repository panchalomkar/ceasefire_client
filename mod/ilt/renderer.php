...
class mod_ilt_renderer extends plugin_renderer_base {
    ...
    public function print_session_list_table($customfields, $sessions, $viewattendees, $editsessions) {
        ...
        $tableheader[] = get_string('options', 'ilt');
        ...
        foreach ($sessions as $session) {
            ...
            if ($editsessions) {
                $options .= $this->output->action_icon(new moodle_url('sessions.php', array('s' => $session->id)),
                        new pix_icon('t/edit', get_string('edit', 'ilt')), null,
                        array('title' => get_string('editsession', 'ilt'))) . ' ';
                $options .= $this->output->action_icon(new moodle_url('sessions.php', array('s' => $session->id, 'c' => 1)),
                        new pix_icon('t/copy', get_string('copy', 'ilt')), null,
                        array('title' => get_string('copysession', 'ilt'))) . ' ';
                $options .= $this->output->action_icon(new moodle_url('sessions.php', array('s' => $session->id, 'd' => 1)),
                        new pix_icon('t/delete', get_string('delete', 'ilt')), null,
                        array('title' => get_string('deletesession', 'ilt'))) . ' ';
                $options .= html_writer::empty_tag('br');
            }
            if ($viewattendees) {
                $options .= html_writer::link('attendees.php?s='.$session->id.'&backtoallsessions='.$session->ilt,
                        get_string('attendees', 'ilt'),
                        array('title' => get_string('seeattendees', 'ilt'))) . html_writer::empty_tag('br');
            }
            if ($isbookedsession) {
                $options .= html_writer::link('signup.php?s='.$session->id.'&backtoallsessions='.$session->ilt,
                        get_string('moreinfo', 'ilt'),
                        array('title' => get_string('moreinfo', 'ilt'))) . html_writer::empty_tag('br');
                if ($session->allowcancellations) {
                    $options .= html_writer::link('cancelsignup.php?s=' . $session->id . '&backtoallsessions=' . $session->ilt,
                        get_string('cancelbooking', 'ilt'), array('title' => get_string('cancelbooking', 'ilt')));
                }
            } else if (!$sessionstarted and !$bookedsession) {
                $options .= html_writer::link('signup.php?s='.$session->id.'&backtoallsessions='.$session->ilt,
                    get_string('signup', 'ilt'));
            }
            if (empty($options)) {
