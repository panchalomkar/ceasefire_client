...
function certificate_get_images($type) {
    global $CFG;
        //tenant wise condition added by Shivkumar on 09052019
        $comppath ="";
    $companyid = certificate_is_company_user();
        if($companyid){$comppath = '_'.$companyid;}

      switch($type) {
          case CERT_IMAGE_BORDER :
              $path = "$CFG->dirroot/mod/certificate/pix".$comppath."/borders";
              $uploadpath = "$CFG->dataroot/mod/certificate/pix".$comppath."/borders";
              break;
          case CERT_IMAGE_SEAL :
              $path = "$CFG->dirroot/mod/certificate/pix".$comppath."/seals";
              $uploadpath = "$CFG->dataroot/mod/certificate/pix".$comppath."/seals";
              break;
          case CERT_IMAGE_SIGNATURE :
              $path = "$CFG->dirroot/mod/certificate/pix".$comppath."/signatures";
              $uploadpath = "$CFG->dataroot/mod/certificate/pix".$comppath."/signatures";
              break;
          case CERT_IMAGE_WATERMARK :
              $path = "$CFG->dirroot/mod/certificate/pix".$comppath."/watermarks";
              $uploadpath = "$CFG->dataroot/mod/certificate/pix".$comppath."/watermarks";
              break;
...
function certificate_print_image($pdf, $certificate, $type, $x, $y, $w, $h) {
    global $CFG;
        ...
        switch($type) {
            case CERT_IMAGE_BORDER :
                $attr = 'borderstyle';
                $path = "$CFG->dirroot/mod/certificate/pix".$comppath."/$type/$certificate->borderstyle";
                $uploadpath = "$CFG->dataroot/mod/certificate/pix".$comppath."/$type/$certificate->borderstyle";
                break;
            case CERT_IMAGE_SEAL :
                $attr = 'printseal';
                $path = "$CFG->dirroot/mod/certificate/pix".$comppath."/$type/$certificate->printseal";
                $uploadpath = "$CFG->dataroot/mod/certificate/pix".$comppath."/$type/$certificate->printseal";
            break;
            case CERT_IMAGE_SIGNATURE :
                $attr = 'printsignature';
                $path = "$CFG->dirroot/mod/certificate/pix".$comppath."/$type/$certificate->printsignature";
                $uploadpath = "$CFG->dataroot/mod/certificate/pix".$comppath."/$type/$certificate->printsignature";
                break;
            case CERT_IMAGE_WATERMARK :
                $attr = 'printwmark';
                $path = "$CFG->dirroot/mod/certificate/pix".$comppath."/$type/$certificate->printwmark";
                $uploadpath = "$CFG->dataroot/mod/certificate/pix".$comppath."/$type/$certificate->printwmark";
                break;
        }
    // Has to be valid
    if (!empty($path)) {
