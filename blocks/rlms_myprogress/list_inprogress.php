...
$PAGE -> set_title(get_string('listinprogresscourses', 'block_rlms_myprogress', ($user->firstname.' '.$user->lastname)));
$PAGE -> set_heading(get_string('listinprogresscourses', 'block_rlms_myprogress', ($user->firstname.' '.$user->lastname)));
...
$PAGE -> navbar -> add(get_string('listinprogresscourses', 'block_rlms_myprogress', ($user->firstname.' '.$user->lastname)));

echo $OUTPUT -> header();

echo $OUTPUT -> heading(get_string('listinprogresscourses', 'block_rlms_myprogress', ($user->firstname.' '.$user->lastname)));
...
foreach ($courses as $course) {
	...
	*/
	...
	if($criteriacomplete > 0 && !$coursecomplete){
		...
		$data[] = date('Y-m-d h:i',$ccompletion->timeenrolled);
		$table->data[] = $data;
	}
}
