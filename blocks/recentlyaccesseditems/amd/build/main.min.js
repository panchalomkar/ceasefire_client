 ...
 */
define("block_recentlyaccesseditems/main", [
  "jquery",
  "block_recentlyaccesseditems/repository",
  "core/templates",
  "core/notification",
], function ($, Repository, Templates, Notification) {
  var SELECTORS_CARDDECK_CONTAINER =
      '[data-region="recentlyaccesseditems-view"]',
    SELECTORS_CARDDECK = '[data-region="recentlyaccesseditems-view-content"]',
    SELECTORS_SHOWMORE_LINK =
      '[data-region="recentlyaccesseditems-view"] [data-action="more-items"]';
  return {
    init: function (root) {
      var limit,
        itemsContainer = (root = $(root)).find(SELECTORS_CARDDECK_CONTAINER),
        itemsContent = root.find(SELECTORS_CARDDECK);
      ((limit = 9), Repository.getRecentItems(limit))
        .then(function (items) {
          var pageContentPromise = (function (root, items) {
            if (items.length > 0) {
              let hasmoreitems = !1;
              return (
                items.length > 3 && (hasmoreitems = !0),
                Templates.render("block_recentlyaccesseditems/view-cards", {
                  items: items,
              ...
              return (
                ...
                items.length > 3 &&
                  (() => {
                    ...
                    //     document
                    //       .querySelector('[data-region="items-list"]')
                    //       .children.forEach(function (hiddenItem) {
